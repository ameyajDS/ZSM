...
-- =============================================================================
--	Procedure	UpdateSewadarApplicationStatus
-- =============================================================================
IF EXISTS	(	SELECT  TOP 1 1 
				FROM    INFORMATION_SCHEMA.ROUTINES 
				WHERE   ROUTINE_TYPE='PROCEDURE'
				AND     SPECIFIC_NAME='UpdateSewadarApplicationStatus'
			)
	BEGIN
		DROP PROCEDURE  dbo.UpdateSewadarApplicationStatus
	END
GO
CREATE PROCEDURE dbo.UpdateSewadarApplicationStatus
(
	@application_id						int,
	@application_status					varchar(20),
	@reason								varchar(200),
...
AS
	SELECT
		...
		application_centre_nm = application_scd.centre_nm,
		sa.application_dt, 
		...
		reference_1_grno = COALESCE(sa.reference_1_grno, ref1_vs.sewadar_grno), 
		sa.reference_1_relationship, 
		reference_1_centre_nm = COALESCE(ref1_scd.centre_nm, ref1_vs.centre_nm),
		reference_1_centre_id = COALESCE(sa.reference_1_centre_id, ref1_vs.satsang_centre_id),
		reference_1_relationship_nm = ref1_rel.relation_nm,
		reference_1_department = COALESCE(sa.reference_1_department, ref1_vs.full_department_nm),
		reference_1_nm = COALESCE(sa.reference_1_nm, ref1_vs.full_nm),
		sa.reference_2_sewadar_id, 
		reference_2_grno = COALESCE(sa.reference_2_grno, ref2_vs.sewadar_grno), 
		sa.reference_2_relationship, 
		reference_2_relationship_nm = ref2_rel.relation_nm,
		reference_2_department = COALESCE(sa.reference_2_department, ref2_vs.full_department_nm),
		reference_2_nm = COALESCE(sa.reference_2_nm, ref2_vs.full_nm),
		reference_2_centre_nm = COALESCE(ref2_scd.centre_nm, ref2_vs.centre_nm),
		reference_2_centre_id = COALESCE(sa.reference_2_centre_id, ref2_vs.satsang_centre_id),
		...
		security_approver_nm = COALESCE(sa.security_approver_nm, sa_vs.full_nm), 
		...
		sewa_samiti_approver_nm = COALESCE(sa.sewa_samiti_approver_nm, ss_vs.full_nm), 
		sa.area_secretary_id, 
		area_secretary_centre_id = COALESCE(sa.area_secretary_centre_id, a_scf.satsang_centre_id),
		area_secretary_centre_nm = area_secretary_scd.centre_nm,
		sa.area_secretary_grno, 
		sa.area_secretary_nm, 
		as_full_nm = a_scf.functionary_nm,
		s_full_nm = s_scf.functionary_nm,
		sa.secretary_id, 
		secretary_centre_id = COALESCE(sa.secretary_centre_id, s_scf.satsang_centre_id),
		...
		sa.sponsorer_grno, 
		sponsorer_nm = COALESCE(sa.sponsorer_nm, sp_vs.full_nm), 
		sponsorer_department =  COALESCE(sa.sponsorer_department, sp_vs.full_department_nm), 
		sa.remarks, 
		sa.reason, 
	...
	LEFT OUTER JOIN CDB..initiation_places ip ON ip.place_id = sa.initiation_place_id
	...
	LEFT OUTER JOIN ViewBasicSatsangCentreDetails application_scd ON application_scd.satsang_centre_id = sa.application_centre_id
