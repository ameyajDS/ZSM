...
IF EXISTS (SELECT * FROM sysobjects WHERE name = 'UploadData' AND xtype = 'P')
	DROP PROCEDURE UploadData
GO
...
AS
	DECLARE
		...
		@old_dept_id			INT,
		@old_sub_dept_id		INT,
		@dbDetails				VARCHAR(100),
		@providerName			VARCHAR(30),
		@command				NVARCHAR(2000),
		@whereClause			VARCHAR(10);
	
BEGIN

	SET @centre_id = @p_satsang_centre_id
	SET @err = 0;
	SET @p_record_cnt = 0;
	SET @p_return_code = 0;
	...
	SET @dbDetails = '''Excel 8.0;Database='+@p_filePath+''''
	SET @providerName = '''Microsoft.Jet.OLEDB.4.0'''
	SET @whereClause = 'Y'
	
	SET @command = 'INSERT INTO #tbl
		...
		FROM OPENROWSET('+@providerName+','+@dbDetails+',
	...
	IF (@@ROWCOUNT > 0)
	BEGIN
		RAISERROR ('Records having Name/DOB overwritten with character(s) other than Y or N', 10, 1);
		SET @p_return_code = -1
		return;
	END
	...
	IF (@@ROWCOUNT > 0)
	BEGIN
		RAISERROR ('Mutiple sewadar records matching... (Names/DOBs which are to be over-written are not considered', 10, 1);
		SET @p_return_code = -2;
		return;
	END
	...
	create table #commentsTable(comments NVARCHAR(2000));
	...
	BEGIN
		...
		SELECT 'Processing ' + CONVERT (VARCHAR, @cnt2) + ' record :: S.No. ' + CONVERT (VARCHAR, @s_no) + ', Sewadar ID: ' + COALESCE (CONVERT (VARCHAR, @sewadar_id), 'XXX') + ', Old GR No. : ' + @old_grno + ', '
				+ 'Gender: ' + COALESCE (@gender, '') + ', Name: ' + COALESCE (@e_frst_nm, '') + ' ' + COALESCE (@e_lst_nm, '') + ', Photo ID :' + @photo_id + ', New GR No. : ' + COALESCE (@new_grno, '') + ', '
				+ 'Dept. : ' + COALESCE (@department_nm, '') + COALESCE (' (' + @sub_department_nm + ')', '') as comment

		IF (@sewadar_id IS NULL)
		BEGIN
			INSERT INTO #CommentsTable
			SELECT 'ERROR: Sewadar ID could NOT be found for the Centre on the basis of Photo ID and considering Gender'
			SET @err = 1;
		END
		...
		BEGIN
			INSERT INTO #CommentsTable
			SELECT 'ERROR: New Sewadar G.R.No. could NOT be found'
			SET @err = 1;
		END
		...
		BEGIN
			...
			BEGIN
				INSERT INTO #CommentsTable
				SELECT 'ERROR: First Name in the Excel File (' + @e_frst_nm + ') does not match with the System (' + @s_frst_nm + ')';
				SET @err = 1;
			END
			...
			BEGIN
				INSERT INTO #CommentsTable
				SELECT '	ERROR: Middle Name in the Excel File (' + COALESCE (@e_mddl_nm, 'NULL') + ') does not match with the System (' + COALESCE (@s_mddl_nm, 'NULL') + ')';
				SET @err = 1;
			END
			IF (@e_lst_nm != @s_lst_nm)
			BEGIN
				INSERT INTO #CommentsTable
				SELECT 'ERROR: Last Name in the Excel File (' + @e_lst_nm + ') does not match with the System (' + @s_lst_nm + ')';
				SET @err = 1;
			END
		END
		...
		BEGIN
			INSERT INTO #CommentsTable
				SELECT 'ERROR: Date of Birth in the Excel File (' + CONVERT (VARCHAR, @e_dob, 106) + ') does not match with the System (' + CONVERT (VARCHAR, @s_dob, 106) + ')';
			SET @err = 1;
		END
		...
		BEGIN
			INSERT INTO #CommentsTable
				SELECT 'ERROR: Difference of Dates of Birth in the Excel File (' + CONVERT (VARCHAR, @e_dob, 106) + ') and the System (' + CONVERT (VARCHAR, @s_dob, 106) + ') is more than three years (1115 days)';
			SET @err = 1;
		END
		...
		BEGIN
			INSERT INTO #CommentsTable
				SELECT 'ERROR: Department could not be found'
			SET @err = 1;
		END
		ELSE IF (@ds_id IS NULL)
		BEGIN
			INSERT INTO #CommentsTable
				SELECT 'ERROR: Department is NOT applicable for the Centre'
			SET @err = 1;
		END
		...
		IF (@sub_department_nm IS NOT NULL)
			...
			BEGIN
				INSERT INTO #CommentsTable
				SELECT 'ERROR: Sub-Department could not be found'
				SET @err = 1;
			END
			ELSE IF (@sds_id IS NULL)
			BEGIN
				INSERT INTO #CommentsTable
				SELECT 'ERROR: Sub-Department is NOT applicable for the Centre'
				SET @err = 1;
			END
		...
		BEGIN
			INSERT INTO #CommentsTable
				SELECT 'ERROR: Sewadar born before 1-Jan-1946 (' + CONVERT (VARCHAR, @s_dob, 106) + ') has the status other than Retired (' + @status_cd + ')'
			SET @err = 1;
		END
		IF EXISTS	(	SELECT	1
			...
			BEGIN
				INSERT INTO #CommentsTable
					SELECT 'ERROR: Photo ID (' + @photo_id + ') for Sewadar already exists'
				SET @err = 1;
			END
