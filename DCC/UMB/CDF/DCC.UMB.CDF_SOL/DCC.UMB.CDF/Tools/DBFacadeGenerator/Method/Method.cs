...
namespace DCC.UMB.CDF.Tools.DBFacadeGenerator.Method
{
    ...
    class SQLObjectReturningTable
    {
        ...
        public void Print(CodeWriter writer)
        {
            ...
            foreach (DataColumn col in returnDetails.Columns)
            {
                ...
                writer.WriteLine(string.Format("public Cndn {0} = new Cndn(\"{0}\", typeof({1}));", col.ColumnName, col.DataType.ToString().Replace("System.", "")));
            }
            ...
            foreach (DataColumn col in returnDetails.Columns)
            {
                writer.WriteLine(string.Format("lst.Add({0});", col.ColumnName));
            }
    ...
    class Method
    {
        ...
        internal void Print(CodeWriter writer, MethodType methodType, bool asTask)
        {
            ...
            {
                CreateMethodSignagureForParameterCreation(writer);
                writer.StartBlock();
        ...
        private void CreateMethodSignagureForParameterCreation(CodeWriter writer)
        {
        ...
        private void CreateMethodBodyForParametersGenerator(CodeWriter writer)
        {
            ...
            foreach (MethodParameter param in this.parameters)
            {
                ...
                {
                    //sqlParameters.Add(csp("a", a, SqlDbType.Int, ParameterDirection.Input));
            ...
            if (!isTask)
            {
                switch (methodType)
                {
                    ...
                    case MethodType.ScalarFunction:
                        ...
                        SqlDbType sqlDbType = (SqlDbType)Enum.Parse(typeof(SqlDbType), scalarFunctionReturnTypeName, true);
                        Type scalarFunctionReturnType = TypeConvertor.ToNetType(sqlDbType);
                        if (scalarFunctionReturnType.IsPrimitive)
                        {
                            writer.WriteLine(
                                string.Format("return (obj == null || obj == DBNull.Value) ? null : ({0}?)obj;",
                                              scalarFunctionReturnType));
                        }
                        else if (scalarFunctionReturnType == typeof(DateTime))
                        {
                            writer.WriteLine(
                                string.Format("return (obj == null || obj == DBNull.Value) ? null : ({0}?)obj;",
                                              scalarFunctionReturnType));
