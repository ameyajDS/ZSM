...
Begin VB.Form frmOccupations 
   ...
   Begin FPSpread.vaSpread sprOccupations 
      Height          =   6465
      Left            =   120
      TabIndex        =   0
      Tag             =   "Occupations Spread"
      Top             =   120
      Width           =   11745
      _Version        =   393216
      _ExtentX        =   20717
      ...
      SpreadDesigner  =   "frmOccupations.frx":0000
   End
End
Attribute VB_Name = "frmOccupations"
...
'The declaration of spread controller for retrieving the records from the occupations
...
Dim WithEvents sprOccupationsController As clsSpreadController
Attribute sprOccupationsController.VB_VarHelpID = -1

'This enum holds all the column numbers of the spread controller 'sprOccupationsController' that
...
Private Enum occupationColumns
    code = 1
    Name = 2
    txn_ts = 3
    effectivefrom = 4
...
On Error GoTo errortag
    ...
    If Not sprOccupationsController.prepareSpreadForSave Then
        Exit Sub
    End If
    ...
    saveData = False
    strDeleteTemplate = "CDB..delOccupations " & "@@,@@"
    strUpdateTemplate = "CDB..insUpdOccupations " & "@@, @@, @@, @@, @@, @@, 1"     '1 - UPDATE
    strInsertTemplate = "CDB..insUpdOccupations " & "@@, @@, @@, @@, NULL, NULL, 0" '0 - INSERT
           
    dbManager.beginTrans
    Dim i As Long
    If sprOccupationsController.getNumberOfRowsForState(DELETED_STATE) <> 0 Then
        
        Dim deleteStrings() As String
        deleteStrings = sprOccupationsController.getDeleteStrings(strDeleteTemplate, _
                                                                            "Yes", _
                                                                            "No ", _
                                                                            occupationColumns.ID, _
                                                                            occupationColumns.txn_ts)
        ...
        Next i
    End If
    If sprOccupationsController.getNumberOfRowsForState(UPDATED_STATE) <> 0 Then
         Dim updateStrings() As String
        updateStrings = sprOccupationsController.getUpdateStrings(strUpdateTemplate, _
                                                                            "Yes", _
                                                                            "No ", _
                                                                            occupationColumns.code, _
                                                                            occupationColumns.Name, _
                                                                            occupationColumns.effectivefrom, _
                                                                            occupationColumns.effectiveTill, _
                                                                            occupationColumns.ID, _
                                                                            occupationColumns.txn_ts)
        ...
        Next i
    End If
    If sprOccupationsController.getNumberOfRowsForState(NEW_STATE) <> 0 Then
        Dim insertStrings() As String
        insertStrings = sprOccupationsController.getInsertStrings(strInsertTemplate, _
                                                                            "Yes", _
                                                                            "No ", _
                                                                            occupationColumns.code, _
                                                                            occupationColumns.Name, _
                                                                            occupationColumns.effectivefrom, _
                                                                            occupationColumns.effectiveTill)
...
'retrieve the existing records of occupations to the spread controller "sprOccupationsController"
Private Sub retrieveOccupations()
On Error GoTo errortag
    Dim strSQL As String
    strSQL = "SELECT occupation_cd, occupation_nm, Cast(txn_ts As BigInt), Convert(varchar, effective_from, 106)effective_from, " & _
             "Convert(varchar, effective_till, 106)effective_till, occupation_id " & _
             "From occupations"
    sprOccupationsController.retrieveRecords strSQL
Exit Sub
    
errortag:
    errorManager.setErrorAttributes modStandardMessages.UnknownError, "retrieveOccupations", "", ERROR_TYPE, Me.clsWindowInterface_getScreenID, Me.clsWindowInterface_getMode
    Err.Raise Err.Number
End Sub
...
Public Function clsWindowInterface_getScreenID() As String
    clsWindowInterface_getScreenID = "PER_04"
End Function
...
Private Sub setSpreadController()
    ...
    Set sprOccupationsController = New clsSpreadController
    sprOccupationsController.registerSpreadInstance uiManager, sprOccupations, TABULAR_SPREAD, Me
    sprOccupationsController.setHeaderHeight 500
    
    sprOccupationsController.addColumn "Occ. Code", 9, edit_CELL_TYPE, True, True, , , , 20
    sprOccupationsController.addColumn "Occupation Name", 60, edit_CELL_TYPE, True, True, , , , 20
    sprOccupationsController.addColumn "txn_ts", 0, STATIC_TEXT_CELL_TYPE, True, NON_MANDATORY_COLUMN
    sprOccupationsController.addColumn "Effective From", 13, DATE_CELL_TYPE, True, True, , , , , CENTER_ALIGNMENT
    sprOccupationsController.addColumn "Effective Till", 13, DATE_CELL_TYPE, True, True, , , , , CENTER_ALIGNMENT
    sprOccupationsController.addColumn "Occ. ID", 5, STATIC_TEXT_CELL_TYPE, False, NON_MANDATORY_COLUMN, , , , , RIGHT_ALIGNMENT
...
Private Sub setEnableDisableControls()
    ...
    Select Case intMode
        Case uiManager.TABULAR_MODE
            sprOccupationsController.setSpreadModeForTabularSpread intMode
    End Select
    
End Sub

Private Sub sprOccupationsController_setDefaultValues(ByVal p_lonRow As Long)
    sprOccupationsController.setTextOfCell p_lonRow, occupationColumns.effectivefrom, Format(Date, "dd mmm yyyy")
    sprOccupationsController.setTextOfCell p_lonRow, occupationColumns.effectiveTill, "31 Dec 9999"
End Sub
...
Private Sub sprOccupationsController_ValidateCell(ByVal p_lonRow As Long, _
...
On Error GoTo errortag
    ...
    varEffectiveFrom = sprOccupationsController.getTextOfCell(p_lonRow, occupationColumns.effectivefrom)
    varEffectivetill = sprOccupationsController.getTextOfCell(p_lonRow, occupationColumns.effectiveTill)
    
    Select Case p_lonColumn
        Case occupationColumns.effectiveTill
        ...
        Case occupationColumns.effectivefrom
        ...
        Case occupationColumns.Name
            If Trim(sprOccupationsController.getTextOfCell(p_lonRow, p_lonColumn)) = "" Then
                sprOccupationsController.setTextOfCell p_lonRow, p_lonColumn, ""
            End If
...
errortag:
    errorManager.setErrorAttributes modStandardMessages.UnknownError, "sprOccupationsController_ValidateCell", "", ERROR_TYPE, Me.clsWindowInterface_getScreenID, Me.clsWindowInterface_getMode
    Err.Raise Err.Number
End Sub

Private Sub sprOccupationsController_ValidateRow(ByVal p_lonRow As Long, p_bolIsRowValid As Boolean)
    Dim varEffectiveFrom As Variant
    Dim varEffectivetill As Variant
    ...
    If sprOccupationsController.isCellValueRepeatedInColumn(utils.createLongArray(occupationColumns.Name), p_lonRow, True) Then
        logger.logMessage modStandardMessages.ValueRepeated, "Occupation Name", ERROR_MESSAGE, Me.clsWindowInterface_getScreenID, Me.clsWindowInterface_getMode
        p_bolIsRowValid = False
    End If
End Sub
