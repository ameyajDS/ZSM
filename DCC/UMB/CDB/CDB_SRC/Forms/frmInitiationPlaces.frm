...
Begin VB.Form frmInitiationPlaces 
   BackColor       =   &H00C0C0C0&
   ...
   Begin VB.Frame fraInitiationPlaces 
      BackColor       =   &H00C0C0C0&
      BorderStyle     =   0  'None
      Height          =   9165
      Left            =   0
      ...
      Top             =   0
      Width           =   15045
      Begin FPSpread.vaSpread sprInitiationPlaces 
         Height          =   5925
         Left            =   360
         TabIndex        =   1
         Top             =   360
         ...
         SpreadDesigner  =   "frmInitiationPlaces.frx":0000
      End
   End
End
Attribute VB_Name = "frmInitiationPlaces"
...
Dim WithEvents sprInitiationPlacesController As clsSpreadController
Attribute sprInitiationPlacesController.VB_VarHelpID = -1

'This enum holds all the column numbers of the spread controller 'sprInitiationPlacesController' that
...
Private Enum initiationPlacesColumns
    Name = 1
    txn_ts = 2
    effectivefrom = 3
    effectiveTill = 4
...
On Error GoTo errortag
    ...
    If Not sprInitiationPlacesController.prepareSpreadForSave Then
        Exit Sub
    End If
    ...
    saveData = False
    strDeleteTemplate = "CDB..delInitiationPlace " & "@@, @@"
    strUpdateTemplate = "CDB..updInitiationPlace " & "@@, @@, @@, @@, @@"
    strInsertTemplate = "CDB..insInitiationPlace " & "@@, @@, @@"
    
    dbManager.beginTrans
    Dim i As Long
    Dim sqlStrings() As String
    If sprInitiationPlacesController.getNumberOfRowsForState(DELETED_STATE) <> 0 Then
         
        sqlStrings = sprInitiationPlacesController.getDeleteStrings(strDeleteTemplate, _
                                                                      "Yes", _
                                                                      "No ", _
                                                                      initiationPlacesColumns.ID, _
                                                                      initiationPlacesColumns.txn_ts)
        ...
        Next i
    End If
    If sprInitiationPlacesController.getNumberOfRowsForState(UPDATED_STATE) <> 0 Then
         sqlStrings = sprInitiationPlacesController.getUpdateStrings(strUpdateTemplate, _
                                                                      "Yes", _
                                                                      "No ", _
                                                                      initiationPlacesColumns.ID, _
                                                                      initiationPlacesColumns.Name, _
                                                                      initiationPlacesColumns.effectivefrom, _
                                                                      initiationPlacesColumns.effectiveTill, _
                                                                      initiationPlacesColumns.txn_ts)
        ...
        Next i
    End If
    If sprInitiationPlacesController.getNumberOfRowsForState(NEW_STATE) <> 0 Then
         sqlStrings = sprInitiationPlacesController.getInsertStrings(strInsertTemplate, _
                                                                      "Yes", _
                                                                      "No ", _
                                                                      initiationPlacesColumns.Name, _
                                                                      initiationPlacesColumns.effectivefrom, _
                                                                      initiationPlacesColumns.effectiveTill)
...
'retrieve the existing records of skills to the spread controller "sprInitiationPlacesController"
Private Sub retrieveInitiationPlaces()
On Error GoTo errortag
    Dim strSQL As String
    strSQL = "SELECT place_nm, Cast(txn_ts As BigInt), Convert(varchar, effective_from, 106)effective_from, " & _
             "Convert(varchar,effective_till,106)effective_till, place_id " & _
             "From CDB..initiation_places"
    sprInitiationPlacesController.retrieveRecords strSQL
Exit Sub
...
Public Function clsWindowInterface_getScreenID() As String
    clsWindowInterface_getScreenID = "LOC_06"
End Function
...
Private Sub setSpreadController()
    ...
    'initializing and adding columns to sprInitiationPlacesController of skills
    Set sprInitiationPlacesController = New clsSpreadController
    sprInitiationPlacesController.registerSpreadInstance uiManager, sprInitiationPlaces, TABULAR_SPREAD, Me
    sprInitiationPlacesController.setHeaderHeight 500
    
    sprInitiationPlacesController.addColumn "Initiation Place", 60, edit_CELL_TYPE, True, True, , , , 20
    sprInitiationPlacesController.addColumn "Txn Ts", 0, STATIC_TEXT_CELL_TYPE, NON_EDITABLE_COLUMN, NON_MANDATORY_COLUMN
    sprInitiationPlacesController.addColumn "Effective From", 15, DATE_CELL_TYPE, True, True, , , , , CENTER_ALIGNMENT
    sprInitiationPlacesController.addColumn "Effective Till", 15, DATE_CELL_TYPE, True, True, , , , , CENTER_ALIGNMENT
    sprInitiationPlacesController.addColumn "ID", 9, STATIC_TEXT_CELL_TYPE, False, False, , , , , RIGHT_ALIGNMENT
...
Private Sub setEnableDisableControls(p_intMode As Integer)
    Select Case p_intMode
        Case uiManager.TABULAR_MODE
            sprInitiationPlacesController.setSpreadModeForTabularSpread p_intMode
        Case uiManager.NEW_MODE
            sprInitiationPlacesController.setSpreadModeForTabularSpread p_intMode
    End Select
End Sub

Private Sub sprInitiationPlacesController_setDefaultValues(ByVal p_lonRow As Long)
    sprInitiationPlacesController.setTextOfCell p_lonRow, initiationPlacesColumns.effectivefrom, Format(Date, "dd mmm yyyy")
    sprInitiationPlacesController.setTextOfCell p_lonRow, initiationPlacesColumns.effectiveTill, "31 Dec 9999"
End Sub
...
Private Sub sprInitiationPlacesController_ValidateCell(ByVal p_lonRow As Long, _
...
On Error GoTo errortag
    ...
    varEffectiveFrom = sprInitiationPlacesController.getTextOfCell(p_lonRow, initiationPlacesColumns.effectivefrom)
    varEffectivetill = sprInitiationPlacesController.getTextOfCell(p_lonRow, initiationPlacesColumns.effectiveTill)
    
    Select Case p_lonColumn
        Case initiationPlacesColumns.effectiveTill
        ...
        Case initiationPlacesColumns.effectivefrom
        ...
        Case initiationPlacesColumns.Name
            If Trim(sprInitiationPlacesController.getTextOfCell(p_lonRow, p_lonColumn)) = "" Then
                sprInitiationPlacesController.setTextOfCell p_lonRow, p_lonColumn, ""
            End If
...
errortag:
    errorManager.setErrorAttributes modStandardMessages.UnknownError, "sprInitiationPlacesController_ValidateCell", "", ERROR_TYPE, Me.clsWindowInterface_getScreenID, Me.clsWindowInterface_getMode
    Err.Raise Err.Number
End Sub


Private Sub sprInitiationPlacesController_ValidateRow(ByVal p_lonRow As Long, p_bolIsRowValid As Boolean)
    Dim varEffectiveFrom As Variant
    Dim varEffectivetill As Variant
    ...
    If sprInitiationPlacesController.isCellValueRepeatedInColumn(utils.createLongArray(initiationPlacesColumns.Name), p_lonRow, True) Then
