...
Begin VB.Form frmQualifications 
   BackColor       =   &H00C0C0C0&
   ...
   Begin VB.Frame fraQualifications 
      BackColor       =   &H00C0C0C0&
      BorderStyle     =   0  'None
      ForeColor       =   &H00800000&
      Height          =   8205
      ...
      Top             =   0
      Width           =   16275
      Begin EditLib.fpText txtQualificationCode 
         Height          =   350
         Left            =   5880
         TabIndex        =   3
         Top             =   480
         ...
         OLEDragMode     =   0
      End
      Begin EditLib.fpText txtQualificationName 
         Height          =   350
         Left            =   3720
         TabIndex        =   5
         Top             =   1220
      ...
      Begin VB.Label lblEffectiveFrom 
         ...
         Width           =   1290
      End
      Begin VB.Label lblQualificationName 
         AutoSize        =   -1  'True
         BackColor       =   &H00C0C0C0&
         Caption         =   "Name "
         ForeColor       =   &H00800000&
         ...
         Width           =   570
      End
      Begin VB.Label lbQualificationCode 
         AutoSize        =   -1  'True
         BackColor       =   &H00C0C0C0&
         Caption         =   "Code "
         ForeColor       =   &H00800000&
      ...
      End
   End
   Begin VB.Frame fraListQualifications 
      BackColor       =   &H00C0C0C0&
      BorderStyle     =   0  'None
      ForeColor       =   &H00800000&
      Height          =   8445
      ...
      Top             =   0
      Width           =   13875
      Begin FPSpread.vaSpread sprListQualifications 
         Height          =   6405
         Left            =   210
         TabIndex        =   11
         TabStop         =   0   'False
         Tag             =   "Qualifications List Spread"
         Top             =   120
         Width           =   11595
         _Version        =   393216
         _ExtentX        =   20452
         ...
         SpreadDesigner  =   "frmQualifications.frx":0000
      End
   End
End
Attribute VB_Name = "frmQualifications"
...
Dim WithEvents sprListQualificationsController As clsSpreadController
Attribute sprListQualificationsController.VB_VarHelpID = -1

'This enum holds all the column numbers of the spread controller 'sprListQualificationsController' that shows all the
...
Private Enum qualificationColumns
    code = 1
    Name = 2
    txn_ts = 3
    effectivefrom = 4
...
'private function returning true when the field - qualification code is valid, otherwise false
Private Function validateQualificationCode() As Boolean
    validateQualificationCode = True
    
    'check for mandatory field
    If (intMode <> uiManager.QUERY_MODE And Trim(Me.txtQualificationCode.Text) = "") Then
        Me.txtQualificationCode.SetFocus
        logger.logMessage modStandardMessages.ValueReqd, "Qualification Code", WARNING_MESSAGE, clsWindowInterface_getScreenID, clsWindowInterface_getMode
        validateQualificationCode = False
    End If
End Function

'private function returning true when the field - qualification name is valid, otherwise false
Private Function validateQualificationName() As Boolean
    validateQualificationName = True

    'check for mandatory field
    If intMode <> uiManager.QUERY_MODE And Trim(Me.txtQualificationName.Text) = "" Then
        Me.txtQualificationName.SetFocus
        logger.logMessage modStandardMessages.ValueReqd, "Qualification Name", WARNING_MESSAGE, clsWindowInterface_getScreenID, clsWindowInterface_getMode
        validateQualificationName = False
    End If
End Function
...
Public Function clsEditInterface_isReadyToSave() As Boolean
    ...
    If (Not validateQualificationCode) Then
        clsEditInterface_isReadyToSave = False
        Exit Function
    ElseIf (Not validateQualificationName) Then
        clsEditInterface_isReadyToSave = False
        Exit Function
...
On Error GoTo errortag
    ...
    strSQL = "CDB..insUpdQualifications '" & Trim(UCase(Me.txtQualificationCode.Text)) & "','" & _
                                             Replace(Trim(Me.txtQualificationName.Text), "'", "''") & "','" & _
    ...
    Select Case intMode
        Case uiManager.INSERT_MODE
            ...
            Dim rsQualifications As clsRecordHashMap
            Set rsQualifications = dbManager.executeSQLGetRS(strSQL)
            
            dbManager.commitTrans
            
            logger.logMessage modStandardMessages.ObjectSaveMsg, "Qualification - '" & Trim(Me.txtQualificationName.Text) & "'|" & rsQualifications.getFieldValueByFieldIndex(0, 0), PLAIN_MESSAGE, Me.clsWindowInterface_getScreenID, Me.clsWindowInterface_getMode
        ...
        Case uiManager.BROWSE_MODE
            ...
            dbManager.commitTrans
            sprListQualificationsController.updateRow Me.clsQueryInterface_getSQLStringForRetrieval, _
                                                      Me.sprListQualifications.ActiveRow
            Me.clsQueryInterface_populateFieldsFromRetrievalSpread Me.sprListQualifications.ActiveRow
            varTxnTs = sprListQualificationsController.getTextOfCell(Me.sprListQualifications.ActiveRow, qualificationColumns.txn_ts)
...
On Error GoTo errortag
    dbManager.beginTrans
    dbManager.executeSQL "CDB..delQualifications " & Trim(Me.txtID.Text) & "," & varTxnTs
    dbManager.commitTrans
Exit Sub
...
'the retrieval is done in the "sprListQualificationsController", so that is returned to the caller.
Public Function clsQueryInterface_getSpreadControllerForRetrieval() As ADL.clsSpreadController
    Set clsQueryInterface_getSpreadControllerForRetrieval = sprListQualificationsController
End Function
...
Public Function clsQueryInterface_getSQLStringForRetrieval() As String
    ...
    Dim qryQualifications As String
    
    qryQualifications = "SELECT qualification_cd, qualification_nm, Cast(txn_ts As BigInt), " & _
                        "Convert(varchar, effective_from, 106)effective_from, " & _
                        "Convert(varchar, effective_till, 106)effective_till, qualIFication_id " & _
                        "FROM Qualifications "

    If Trim(Me.txtQualificationCode.Text) <> "" Then
        qryQualifications = qryQualifications & "WHERE qualification_cd LIKE '%" & Trim(Me.txtQualificationCode.Text) & "%' AND "
    Else
        qryQualifications = qryQualifications & "WHERE qualification_cd LIKE '%%' AND "
    End If

    If Trim(Me.txtQualificationName.Text) <> "" Then
        qryQualifications = qryQualifications & "qualIFication_nm LIKE '%" & Replace(Trim(Me.txtQualificationName.Text), "'", "''") & "%' AND "
    End If

    If Trim(Me.txtEffectiveFrom.Text) <> "" Then
        qryQualifications = qryQualifications & "effective_from = '" & Trim(Me.txtEffectiveFrom.Text) & "' AND "
    End If

    If Trim(Me.txtEffectiveTill.Text) <> "" Then
        qryQualifications = qryQualifications & "effective_till = '" & Trim(Me.txtEffectiveTill.Text) & "' AND "
    End If

    If Trim(Me.txtID.Text) <> "" Then
        qryQualifications = qryQualifications & "qualification_id = " & Trim(Me.txtID.Text) & " AND "
    End If
    
    qryQualifications = Left(Trim(qryQualifications), Len(Trim(qryQualifications)) - 3)
    qryQualifications = qryQualifications & " ORDER BY qualIFication_id"
    
    clsQueryInterface_getSQLStringForRetrieval = qryQualifications
...
Public Sub clsQueryInterface_populateFieldsFromRetrievalSpread(p_lonRow As Long)
    ...
    Me.txtQualificationCode.Text = Trim(CStr(sprListQualificationsController.getTextOfCell(p_lonRow, qualificationColumns.code)))
    Me.txtQualificationName.Text = CStr(sprListQualificationsController.getTextOfCell(p_lonRow, qualificationColumns.Name))
    varTxnTs = sprListQualificationsController.getTextOfCell(p_lonRow, qualificationColumns.txn_ts)
    Me.txtEffectiveFrom.Text = Format(CDate(sprListQualificationsController.getTextOfCell(p_lonRow, qualificationColumns.effectivefrom)), "dd-mmm-yyyy")
    Me.txtEffectiveTill.Text = Format(CDate(sprListQualificationsController.getTextOfCell(p_lonRow, qualificationColumns.effectiveTill)), "dd-mmm-yyyy")
    Me.txtID.Text = sprListQualificationsController.getTextOfCell(p_lonRow, qualificationColumns.ID)
...
Public Sub clsWindowInterface_fireErase()
    ...
    utils.clearControls Me.txtID, Me.txtQualificationCode, Me.txtQualificationName
    ...
    If Not Me.sprListQualifications Is Nothing Then
        sprListQualificationsController.clearSpread
    End If
...
Public Function clsWindowInterface_getScreenID() As String
    clsWindowInterface_getScreenID = "PER_05"
End Function
...
Private Sub setEnableDisableControls()
    ...
    Select Case intMode
        ...
        Case uiManager.INSERT_MODE
            ...
            sprListQualificationsController.setSpreadMode intMode
            Me.fraQualifications.ZOrder 0
            Me.txtQualificationCode.SetFocus
        ...
        Case uiManager.LIST_MODE
            ...
            sprListQualificationsController.setSpreadMode intMode
            Me.fraListQualifications.ZOrder 0
...
Private Sub setSpreadControllers()
    ...
    Set sprListQualificationsController = New clsSpreadController
    sprListQualificationsController.registerSpreadInstance uiManager, Me.sprListQualifications, LIST_SPREAD, Me
    sprListQualificationsController.setHeaderHeight 600
    
    sprListQualificationsController.addColumn "Code", 12, STATIC_TEXT_CELL_TYPE, False, True
    sprListQualificationsController.addColumn "Qualification", 50, STATIC_TEXT_CELL_TYPE, False, True
    sprListQualificationsController.addColumn "TXN_TS", 0, STATIC_TEXT_CELL_TYPE, False, True
    sprListQualificationsController.addColumn "Effective From", 15, STATIC_TEXT_CELL_TYPE, False, True, , , , , CENTER_ALIGNMENT
    sprListQualificationsController.addColumn "Effective Till", 15, STATIC_TEXT_CELL_TYPE, False, True, , , , , CENTER_ALIGNMENT
    sprListQualificationsController.addColumn "ID", 8, STATIC_TEXT_CELL_TYPE, False, True, , , , , RIGHT_ALIGNMENT
End Sub
...
Private Sub Form_Load()
    modControlsSettings.setStandardControlSpecifications Me.lblEffectiveTill, Me.lblId, Me.lblQualificationName, Me.lbQualificationCode, Me.lblEffectiveFrom, _
                                                         Me.txtEffectiveFrom, Me.txtEffectiveTill, Me.txtID, Me.txtQualificationCode, Me.txtQualificationName
    modControlsSettings.setStateToNormal Me.txtEffectiveFrom, Me.txtEffectiveTill, Me.txtID, Me.txtQualificationCode, Me.txtQualificationName
End Sub
...
'The method sets the status text of the main screen
Private Sub txtQualificationCode_GotFocus()
    uiManager.setStatusText ""
End Sub

Private Sub txtQualificationCode_Validate(Cancel As Boolean)
    validateQualificationCode
End Sub

'The method sets the status text of the main screen
Private Sub txtQualificationName_GotFocus()
    uiManager.setStatusText ""
End Sub

Private Sub txtQualificationName_Validate(Cancel As Boolean)
