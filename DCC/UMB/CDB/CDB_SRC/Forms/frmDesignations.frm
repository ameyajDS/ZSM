...
Begin VB.Form frmDesignations 
   ...
   Begin VB.Frame fraDesignation 
      ...
      End
   End
   Begin VB.Frame fraListDesignations 
      BackColor       =   &H00C0C0C0&
      BorderStyle     =   0  'None
      ForeColor       =   &H00800000&
      Height          =   6615
      ...
      Top             =   0
      Width           =   13575
      Begin FPSpread.vaSpread sprListDesignations 
         Height          =   6465
         Left            =   120
         TabIndex        =   18
         TabStop         =   0   'False
         Tag             =   "Designation List Spread"
         Top             =   120
         Width           =   11685
         _Version        =   393216
         _ExtentX        =   20611
         ...
         SpreadDesigner  =   "frmDesignations.frx":0000
      End
   End
End
Attribute VB_Name = "frmDesignations"
...
'stores newly generated designation ID returned in fireSave() method of clsEditInterface. the
...
'This enum holds all the column numbers of the spread controller 'sprListDesignationsController' that shows all the
'records of designations. The column numbers are stored into variables named corresponding to column names. These are
'called in functions of spreadController like 'getTextOfCell' or 'setTextOfCell' which requires row & column number.
Private Enum designationColumns
    code = 1
    Name = 2
    minAge = 3
    maxAge = 4
...
Public Sub clsCalledFormInterface_getReturnValues(p_strDisplayText As String, _
    ...
    p_strDisplayText = Me.txtDesignationName.Text
    
    p_colAdditionalValues.Add intDesignationID, "designation_id"
    p_colAdditionalValues.Add Me.txtDesignationName, "designation_nm"
    p_colAdditionalValues.Add Me.txtEffectiveFrom, "effective_from"
    p_colAdditionalValues.Add Me.txtEffectiveTill, "effective_till"
        
    Select Case intMode
        Case uiManager.NEW_MODE
            p_strIDText = intDesignationID
...
On Error GoTo errortag
    ...
    Select Case intMode
        Case uiManager.UPDATE_MODE, uiManager.REFERENCE_MODE
            ...
            sprListDesignationsController.retrieveRecords Me.clsQueryInterface_getSQLStringForRetrieval
...
'private function returning true when the field - designation code is valid, otherwise false
Private Function validateDesignationCode() As Boolean
    
    validateDesignationCode = True
    
    'check for mandatory field
    If (intMode <> uiManager.QUERY_MODE And Trim(Me.txtDesignationCode.Text) = "") Then
        Me.txtDesignationCode.SetFocus
        logger.logMessage modStandardMessages.ValueReqd, "Designation Code", WARNING_MESSAGE, clsWindowInterface_getScreenID, clsWindowInterface_getMode
        validateDesignationCode = False
    End If
    
End Function

'private function returning true when the field - designation name is valid, otherwise false
Private Function validateDesignationName() As Boolean

    validateDesignationName = True

    'check for mandatory field
    If intMode <> uiManager.QUERY_MODE And Trim(Me.txtDesignationName.Text) = "" Then
        Me.txtDesignationName.SetFocus
        logger.logMessage modStandardMessages.ValueReqd, "Designation Name", WARNING_MESSAGE, clsWindowInterface_getScreenID, clsWindowInterface_getMode
        validateDesignationName = False
    End If
...
'private function returning true when the field - min age is valid, otherwise false
Private Function validateMinAge() As Boolean
    validateMinAge = True
    
    If (intMode <> uiManager.QUERY_MODE And Me.txtMinAge.Text <> "") Then
        If (Me.txtMaxStartingAge.Text <> "") Then
            If (Me.txtMaxStartingAge.Text <= Me.txtMinAge.Text) Then
                Me.txtMinAge.SetFocus
                logger.logMessage modStandardMessages.LesserValueReqd, "Min Age|Max Starting Age", WARNING_MESSAGE, clsWindowInterface_getScreenID, clsWindowInterface_getMode
                validateMinAge = False
                Exit Function
            End If
        End If
        If (Me.txtRetirementAge.Text <> "") Then
            If (Me.txtMinAge.Text <> "" And Me.txtMinAge.Text > Me.txtRetirementAge.Text) Then
                Me.txtMinAge.SetFocus
                logger.logMessage modStandardMessages.LesserValueReqd, "Min Age|Retirement Age", WARNING_MESSAGE, clsWindowInterface_getScreenID, clsWindowInterface_getMode
                validateMinAge = False
            End If
        End If
    End If
...
'private function returning true when the field - max starting age is valid, otherwise false
Private Function validateMaxStartingAge() As Boolean
    validateMaxStartingAge = True
    
    If (intMode <> uiManager.QUERY_MODE And Me.txtMaxStartingAge.Text <> "") Then
        If (Me.txtMinAge.Text <> "") Then
            If (Me.txtMaxStartingAge.Text <= Me.txtMinAge.Text) Then
                Me.txtMaxStartingAge.SetFocus
                logger.logMessage modStandardMessages.GreaterValueReqd, "Max Starting Age|Min Age", WARNING_MESSAGE, clsWindowInterface_getScreenID, clsWindowInterface_getMode
                validateMaxStartingAge = False
                Exit Function
            End If
        End If
        If (Me.txtRetirementAge.Text <> "") Then
            If (Me.txtMaxStartingAge.Text >= Me.txtRetirementAge.Text) Then
                Me.txtMaxStartingAge.SetFocus
                logger.logMessage modStandardMessages.LesserValueReqd, "Max Starting Age|Retirement Age", WARNING_MESSAGE, clsWindowInterface_getScreenID, clsWindowInterface_getMode
                validateMaxStartingAge = False
            End If
        End If
    End If
...
'private function returning true when the field - retirement age is valid, otherwise false
Private Function validateRetirementAge() As Boolean
    validateRetirementAge = True
    
    If (intMode <> uiManager.QUERY_MODE And Me.txtRetirementAge.Text <> "") Then
        If (Me.txtMinAge.Text <> "") Then
            If (Me.txtRetirementAge.Text <= Me.txtMinAge.Text) Then
                Me.txtRetirementAge.SetFocus
                logger.logMessage modStandardMessages.GreaterValueReqd, "Retirement Age|Min Age", WARNING_MESSAGE, clsWindowInterface_getScreenID, clsWindowInterface_getMode
                validateRetirementAge = False
                Exit Function
            End If
        End If
        If (Me.txtMaxStartingAge.Text <> "") Then
            If (Me.txtMaxStartingAge.Text >= Me.txtRetirementAge.Text) Then
                Me.txtRetirementAge.SetFocus
                logger.logMessage modStandardMessages.GreaterValueReqd, "Retirement Age|Max Starting Age", WARNING_MESSAGE, clsWindowInterface_getScreenID, clsWindowInterface_getMode
                validateRetirementAge = False
            End If
        End If
    End If
...
Public Function clsEditInterface_isReadyToSave() As Boolean
    ...
    If (Not validateDesignationCode) Then
        clsEditInterface_isReadyToSave = False
        Exit Function
    ElseIf (Not validateDesignationName) Then
        clsEditInterface_isReadyToSave = False
        Exit Function
    ElseIf (Not validateMinAge) Then
        clsEditInterface_isReadyToSave = False
        Exit Function
    ElseIf (Not validateMaxStartingAge) Then
        clsEditInterface_isReadyToSave = False
        Exit Function
    ElseIf (Not validateRetirementAge) Then
        clsEditInterface_isReadyToSave = False
        Exit Function
...
On Error GoTo errortag
    ...
    strSQL = "CDB..insUpdDesignations '" & Replace(Trim(UCase(Me.txtDesignationCode.Text)), "'", "''") & "','" & _
                                           Replace(Trim(Me.txtDesignationName.Text), "'", "''") & "'," & _
                                           IIf(Me.txtMinAge.Text = "", "NULL", Trim(Me.txtMinAge.Text)) & "," & _
                                           IIf(Me.txtMaxStartingAge.Text = "", "NULL", Trim(Me.txtMaxStartingAge.Text)) & "," & _
                                           IIf(Me.txtRetirementAge.Text = "", "NULL", Trim(Me.txtRetirementAge.Text)) & ",'" & _
    ...
    Select Case intMode
        Case uiManager.INSERT_MODE, uiManager.NEW_MODE
            ...
            Dim rsDesignations As clsRecordHashMap
            Set rsDesignations = dbManager.executeSQLGetRS(strSQL)
            intDesignationID = rsDesignations.getFieldValueByFieldIndex(0, 0)
            
            dbManager.commitTrans
            
            If intMode = uiManager.INSERT_MODE Then
                logger.logMessage modStandardMessages.ObjectSaveMsg, "Designation Name - '" & Trim(Me.txtDesignationName.Text) & "'|" & intDesignationID, PLAIN_MESSAGE, Me.clsWindowInterface_getScreenID, Me.clsWindowInterface_getMode
        ...
        Case uiManager.BROWSE_MODE, uiManager.UPDATE_MODE
            ...
            If intMode = uiManager.BROWSE_MODE Then
                sprListDesignationsController.updateRow Me.clsQueryInterface_getSQLStringForRetrieval, _
                                                        Me.sprListDesignations.ActiveRow
                Me.clsQueryInterface_populateFieldsFromRetrievalSpread Me.sprListDesignations.ActiveRow
                varTxnTs = sprListDesignationsController.getTextOfCell(Me.sprListDesignations.ActiveRow, designationColumns.txn_ts)
...
On Error GoTo errortag
    dbManager.beginTrans
    dbManager.executeSQL "CDB..delDesignations " & Trim(Me.txtID.Text) & "," & varTxnTs
    dbManager.commitTrans
Exit Sub
...
Public Function clsQueryInterface_getSpreadControllerForRetrieval() As ADL.clsSpreadController
    Set clsQueryInterface_getSpreadControllerForRetrieval = sprListDesignationsController
End Function
...
Public Function clsQueryInterface_getSQLStringForRetrieval() As String
    ...
    Dim qryDesignations As String
    
    qryDesignations = "SELECT designation_cd, designation_nm, minimum_age, maximum_starting_age, " & _
                      "retirement_age, Cast(txn_ts As BigInt), Convert(varchar, effective_from, 106)effective_from, " & _
                      "Convert(varchar, effective_till, 106)effective_till, designation_id " & _
                      "FROM designations "
    
    If Trim(Me.txtDesignationCode.Text) <> "" Then
        qryDesignations = qryDesignations & "WHERE designation_cd LIKE '%" & Trim(Me.txtDesignationCode.Text) & "%' AND "
    Else
        qryDesignations = qryDesignations & "WHERE designation_cd LIKE '%%' AND "
    End If

    If Trim(Me.txtDesignationName.Text) <> "" Then
        qryDesignations = qryDesignations & "designation_nm LIKE '%" & Replace(Trim(Me.txtDesignationName.Text), "'", "''") & "%' AND "
    End If

    If Trim(Me.txtMinAge.Text) <> "" Then
        qryDesignations = qryDesignations & "minimum_age = " & Trim(Me.txtMinAge.Text) & " AND "
    End If

    If Trim(Me.txtMaxStartingAge.Text) <> "" Then
        qryDesignations = qryDesignations & "maximum_starting_age = " & Trim(Me.txtMaxStartingAge.Text) & " AND "
    End If

    If Trim(Me.txtRetirementAge.Text) <> "" Then
        qryDesignations = qryDesignations & "retirement_age = " & Trim(Me.txtRetirementAge.Text) & " AND "
    End If

    If Trim(Me.txtEffectiveFrom.Text) <> "" Then
        qryDesignations = qryDesignations & "effective_from = '" & Trim(Me.txtEffectiveFrom.Text) & "' AND "
    End If

    If Trim(Me.txtEffectiveTill.Text) <> "" Then
        qryDesignations = qryDesignations & "effective_till = '" & Trim(Me.txtEffectiveTill.Text) & "' AND "
    End If

    If Trim(Me.txtID.Text) <> "" Then
        qryDesignations = qryDesignations & "designation_id = " & Trim(Me.txtID.Text) & " AND "
    End If
    
    qryDesignations = Left(Trim(qryDesignations), Len(Trim(qryDesignations)) - 3)
    qryDesignations = qryDesignations & " ORDER BY designation_id"
    
    clsQueryInterface_getSQLStringForRetrieval = qryDesignations
...
Public Sub clsQueryInterface_populateFieldsFromRetrievalSpread(p_lonRow As Long)
    ...
    Me.txtDesignationCode.Text = CStr(Trim(sprListDesignationsController.getTextOfCell(p_lonRow, designationColumns.code)))
    Me.txtDesignationName.Text = CStr(sprListDesignationsController.getTextOfCell(p_lonRow, designationColumns.Name))
    Me.txtMinAge.Text = sprListDesignationsController.getTextOfCell(p_lonRow, designationColumns.minAge)
    Me.txtMaxStartingAge.Text = sprListDesignationsController.getTextOfCell(p_lonRow, designationColumns.maxAge)
    Me.txtRetirementAge.Text = sprListDesignationsController.getTextOfCell(p_lonRow, designationColumns.retirementAge)
    varTxnTs = sprListDesignationsController.getTextOfCell(p_lonRow, designationColumns.txn_ts)
    Me.txtEffectiveFrom.Text = Format(CDate(sprListDesignationsController.getTextOfCell(p_lonRow, designationColumns.effectivefrom)), "dd-mmm-yyyy")
    Me.txtEffectiveTill.Text = Format(CDate(sprListDesignationsController.getTextOfCell(p_lonRow, designationColumns.effectiveTill)), "dd-mmm-yyyy")
    Me.txtID.Text = sprListDesignationsController.getTextOfCell(p_lonRow, designationColumns.ID)
...
Public Sub clsWindowInterface_fireErase()
    ...
    utils.clearControls Me.txtDesignationCode, Me.txtDesignationName, Me.txtMaxStartingAge, Me.txtID, _
                                  Me.txtMaxStartingAge, Me.txtMinAge, Me.txtRetirementAge
    ...
    If Not Me.sprListDesignations Is Nothing Then
        sprListDesignationsController.clearSpread
    End If
...
Public Function clsWindowInterface_getScreenID() As String
    clsWindowInterface_getScreenID = "PER_03"
End Function
...
Private Sub setSpreadController()
    ...
    'initializing and adding columns to sprListDesignationsController of designations
    
    Set sprListDesignationsController = New clsSpreadController
    sprListDesignationsController.registerSpreadInstance uiManager, Me.sprListDesignations, LIST_SPREAD, Me
    sprListDesignationsController.setHeaderHeight 600
                                         
    sprListDesignationsController.addColumn "Desg. Code", 8, STATIC_TEXT_CELL_TYPE, False, True
    sprListDesignationsController.addColumn "Designation Name", 42, STATIC_TEXT_CELL_TYPE, False, True
    sprListDesignationsController.addColumn "Min. Age", 6, STATIC_TEXT_CELL_TYPE, False, False, , , , , RIGHT_ALIGNMENT
    sprListDesignationsController.addColumn "Max. Age", 6, STATIC_TEXT_CELL_TYPE, False, False, , , , , RIGHT_ALIGNMENT
    sprListDesignationsController.addColumn "Retirement Age", 10, STATIC_TEXT_CELL_TYPE, False, False, , , , , RIGHT_ALIGNMENT
    sprListDesignationsController.addColumn "Txn Ts", 0, STATIC_TEXT_CELL_TYPE, False, True, , , , , RIGHT_ALIGNMENT
    sprListDesignationsController.addColumn "Effective    From", 10, STATIC_TEXT_CELL_TYPE, False, True
    sprListDesignationsController.addColumn "Effective    Till", 10, STATIC_TEXT_CELL_TYPE, False, True
    sprListDesignationsController.addColumn "Desg. ID", 7, STATIC_TEXT_CELL_TYPE, False, True, , , , , RIGHT_ALIGNMENT
...
Private Sub setEnableDisableControls()
    ...
    Select Case intMode
        ...
        Case uiManager.REFERENCE_MODE
            ...
            modControlsSettings.setStateToReadOnly Me.txtDesignationCode, Me.txtDesignationName, Me.txtEffectiveFrom, Me.txtEffectiveTill, Me.txtID, Me.txtMaxStartingAge, Me.txtMinAge, Me.txtRetirementAge
            sprListDesignationsController.setSpreadMode intMode
            Me.fraDesignation.ZOrder 0
        ...
        Case uiManager.LIST_MODE
            ...
            sprListDesignationsController.setSpreadMode intMode
            Me.fraListDesignations.ZOrder 0
...
Private Sub Form_Load()
    modControlsSettings.setStandardControlSpecifications Me.lblDesignationCode, Me.lblDesignationName, Me.lblEffectiveTill, Me.lblId, Me.lblMaximumStartingAge, Me.lblMinimumAge, Me.lblRetirementAge, Me.lblEffectiveFrom, _
                                                         Me.txtDesignationCode, Me.txtDesignationName, Me.txtEffectiveFrom, Me.txtEffectiveTill, Me.txtID, Me.txtMaxStartingAge, Me.txtMinAge, Me.txtRetirementAge
    modControlsSettings.setStateToNormal Me.txtDesignationCode, Me.txtDesignationName, Me.txtEffectiveFrom, Me.txtEffectiveTill, Me.txtID, Me.txtMaxStartingAge, Me.txtMinAge, Me.txtRetirementAge
End Sub
...
Private Sub txtDesignationCode_Validate(Cancel As Boolean)
...
'The method sets the status text of the main screen
Private Sub txtMaxStartingAge_GotFocus()
    Select Case intMode
        Case uiManager.INSERT_MODE, uiManager.NEW_MODE, uiManager.QUERY_MODE, uiManager.BROWSE_MODE, uiManager.UPDATE_MODE
            uiManager.setStatusText "Please enter age less than retirement age and greater than min. Age"
        Case uiManager.REFERENCE_MODE, uiManager.RETRIEVE_MODE
            uiManager.setStatusText ""
    End Select
End Sub

Private Sub txtMaxStartingAge_Validate(Cancel As Boolean)
    validateMaxStartingAge
End Sub

'The method sets the status text of the main screen
Private Sub txtMinAge_GotFocus()
    Select Case intMode
        Case uiManager.INSERT_MODE, uiManager.NEW_MODE, uiManager.QUERY_MODE, uiManager.BROWSE_MODE, uiManager.UPDATE_MODE
            uiManager.setStatusText "Please enter age less than retirement age and max. Age"
        Case uiManager.REFERENCE_MODE, uiManager.RETRIEVE_MODE
            uiManager.setStatusText ""
    End Select
End Sub

Private Sub txtMinAge_Validate(Cancel As Boolean)
    validateMinAge
End Sub

'The method sets the status text of the main screen
Private Sub txtRetirementAge_GotFocus()
    Select Case intMode
        Case uiManager.INSERT_MODE, uiManager.NEW_MODE, uiManager.QUERY_MODE, uiManager.BROWSE_MODE, uiManager.UPDATE_MODE
            uiManager.setStatusText "Please enter age greater than max. age and Min. Age"
        Case uiManager.REFERENCE_MODE, uiManager.RETRIEVE_MODE
            uiManager.setStatusText ""
    End Select
End Sub

Private Sub txtRetirementAge_Validate(Cancel As Boolean)
