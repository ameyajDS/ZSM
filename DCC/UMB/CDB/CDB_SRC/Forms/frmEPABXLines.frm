...
Begin VB.Form frmEPABXLines 
   ...
   Begin VB.Frame fraEpabxLines 
      ...
      Begin EditLib.fpDateTime txtEffectiveTill 
         ...
         OLEDragMode     =   0
      End
      Begin EditLib.fpText txtExtensionNo 
         Height          =   345
         Left            =   3720
         TabIndex        =   1
         Top             =   480
      ...
      End
   End
   Begin VB.Frame fraListEpabxLines 
      BackColor       =   &H00C0C0C0&
      BorderStyle     =   0  'None
      ForeColor       =   &H00800000&
      Height          =   7905
      ...
      Top             =   0
      Width           =   9495
      Begin FPSpread.vaSpread sprEPABXLines 
         Height          =   6465
         Left            =   3000
         TabIndex        =   7
         TabStop         =   0   'False
         Tag             =   "EPABX Lines List Spread"
         Top             =   120
         Width           =   5175
         _Version        =   393216
         _ExtentX        =   9128
         ...
         SpreadDesigner  =   "frmEPABXLines.frx":0000
      End
   End
End
Attribute VB_Name = "frmEPABXLines"
...
'stores the value of txn_ts
Dim varTxnTs As Variant

'This variable holds the value of the extension number that is already stored in the database. The variable
'is used as reference to save new extension number in browse mode in case the user changes the extension number
...
Dim WithEvents sprEPABXLinesController As clsSpreadController
Attribute sprEPABXLinesController.VB_VarHelpID = -1

'This enum holds all the column numbers of the spread controller 'sprEPABXLinesController' that shows all the
...
Private Enum EpabxLinesColumns
    extensionNo = 1
    txn_ts = 2
    effectivefrom = 3
    effectiveTill = 4
...
Public Sub clsCalledFormInterface_getReturnValues(p_strDisplayText As String, _
    ...
    p_strDisplayText = Me.txtExtensionNo.Text
...
On Error GoTo errortag
    ...
    Select Case intMode
        Case uiManager.UPDATE_MODE, uiManager.REFERENCE_MODE
            Me.txtExtensionNo.Text = p_strDisplayText
            sprEPABXLinesController.retrieveRecords Me.clsQueryInterface_getSQLStringForRetrieval
...
'private function returning true when the field - Extension No is valid, otherwise false
Private Function validateExtensionNo() As Boolean

    validateExtensionNo = True

    'check for mandatory field
    If intMode <> uiManager.QUERY_MODE And Trim(Me.txtExtensionNo.Text) = "" Then
        Me.txtExtensionNo.SetFocus
        logger.logMessage modStandardMessages.ValueReqd, "Extension No", WARNING_MESSAGE, clsWindowInterface_getScreenID, clsWindowInterface_getMode
        validateExtensionNo = False
    End If
...
Public Function clsEditInterface_isReadyToSave() As Boolean
    ...
    'validate event of the last focused contol is called before save
    Me.ValidateControls
    
    If (Not validateExtensionNo) Then
...
On Error GoTo errortag
    ...
    strSQL = "CDB..insUpdEpabxLines '" & Trim(Me.txtExtensionNo.Text) & "','" & _
                                         IIf(strExtensionNo = "", "NULL", strExtensionNo) & "','" & _
    ...
    Select Case intMode
       ...
       Case uiManager.BROWSE_MODE, uiManager.UPDATE_MODE
            ...
            If intMode = uiManager.BROWSE_MODE Then
                dbManager.commitTrans
                sprEPABXLinesController.updateRow Me.clsQueryInterface_getSQLStringForRetrieval, _
                                                  Me.sprEPABXLines.ActiveRow
                Me.clsQueryInterface_populateFieldsFromRetrievalSpread Me.sprEPABXLines.ActiveRow
                varTxnTs = sprEPABXLinesController.getTextOfCell(Me.sprEPABXLines.ActiveRow, EpabxLinesColumns.txn_ts)
                logger.logMessage modStandardMessages.SaveMsg, "", PLAIN_MESSAGE, Me.clsWindowInterface_getScreenID, Me.clsWindowInterface_getMode
            Else
                dbManager.commitTrans
            End If
...
On Error GoTo errortag
    dbManager.beginTrans
    dbManager.executeSQL "CDB..delEpabxLines '" & CStr(Trim(Me.txtExtensionNo.Text)) & "'," & varTxnTs
    dbManager.commitTrans
Exit Sub
...
'the retrieval is done in the "sprEPABXLinesController", so that is returned to the caller.
Public Function clsQueryInterface_getSpreadControllerForRetrieval() As ADL.clsSpreadController
    Set clsQueryInterface_getSpreadControllerForRetrieval = sprEPABXLinesController
End Function
...
Public Function clsQueryInterface_getSQLStringForRetrieval() As String
    ...
    Dim qryEpabxLines As String
    
    qryEpabxLines = "SELECT extension_no, Cast(txn_ts As BigInt), " & _
                    "Convert(varchar, effective_from, 106)effective_from, " & _
                    "Convert(varchar, effective_till, 106)effective_till " & _
                    "FROM epabx_lines "
    
    If Trim(Me.txtExtensionNo.Text) <> "" Then
        qryEpabxLines = qryEpabxLines & "WHERE extension_no LIKE '%" & Trim(Me.txtExtensionNo.Text) & "%' "
    Else
        qryEpabxLines = qryEpabxLines & "WHERE extension_no LIKE '%%' "
    End If

    If Trim(Me.txtEffectiveFrom.Text) <> "" Then
        qryEpabxLines = qryEpabxLines & "AND effective_from = '" & Trim(Me.txtEffectiveFrom.Text) & "' "
    End If

    If Trim(Me.txtEffectiveTill.Text) <> "" Then
        qryEpabxLines = qryEpabxLines & "AND effective_till = '" & Trim(Me.txtEffectiveTill.Text) & "' "
    End If
    
    clsQueryInterface_getSQLStringForRetrieval = qryEpabxLines
...
Public Sub clsQueryInterface_populateFieldsFromRetrievalSpread(p_lonRow As Long)
    ...
    Me.txtExtensionNo.Text = Trim(CStr(sprEPABXLinesController.getTextOfCell(p_lonRow, EpabxLinesColumns.extensionNo)))
    strExtensionNo = Me.txtExtensionNo.Text
    varTxnTs = sprEPABXLinesController.getTextOfCell(p_lonRow, EpabxLinesColumns.txn_ts)
    Me.txtEffectiveFrom.Text = sprEPABXLinesController.getTextOfCell(p_lonRow, EpabxLinesColumns.effectivefrom)
    Me.txtEffectiveTill.Text = sprEPABXLinesController.getTextOfCell(p_lonRow, EpabxLinesColumns.effectiveTill)
...
Public Sub clsWindowInterface_fireErase()
    ...
    If Not Me.sprEPABXLines Is Nothing Then
        sprEPABXLinesController.clearSpread
    End If
...
Public Function clsWindowInterface_getScreenID() As String
    clsWindowInterface_getScreenID = "MIS_03"
End Function
...
Private Sub setEnableDisableControls()
    ...
    Select Case intMode
        ...
        Case uiManager.INSERT_MODE
            ...
            sprEPABXLinesController.setSpreadMode intMode
            Me.fraEpabxLines.ZOrder 0
            Me.txtExtensionNo.SetFocus
        ...
        Case uiManager.REFERENCE_MODE
            ...
            modControlsSettings.setStateToReadOnly Me.txtEffectiveFrom, Me.txtEffectiveTill, Me.txtExtensionNo
            Me.fraEpabxLines.ZOrder 0
            
        Case uiManager.LIST_MODE
                
            sprEPABXLinesController.setSpreadMode intMode
            Me.fraListEpabxLines.ZOrder 0
...
Private Sub setSpreadControllers()
    ...
    Set sprEPABXLinesController = New clsSpreadController
    sprEPABXLinesController.registerSpreadInstance uiManager, Me.sprEPABXLines, LIST_SPREAD, Me
    sprEPABXLinesController.setHeaderHeight 600
    
    sprEPABXLinesController.addColumn "Extension No", 40, STATIC_TEXT_CELL_TYPE, False, True, , , , , RIGHT_ALIGNMENT
    sprEPABXLinesController.addColumn "Txn Ts", 0, STATIC_TEXT_CELL_TYPE, False, True, , , , , RIGHT_ALIGNMENT
    sprEPABXLinesController.addColumn "Effective From", 30, STATIC_TEXT_CELL_TYPE, False, True, , , , , CENTER_ALIGNMENT
    sprEPABXLinesController.addColumn "Effective Till", 30, STATIC_TEXT_CELL_TYPE, False, True, , , , , CENTER_ALIGNMENT
...
Private Sub Form_Load()
    modControlsSettings.setStandardControlSpecifications Me.lblEffectiveFrom, Me.lblEffectiveTill, Me.lblExtensionNo, _
                                                         Me.txtEffectiveFrom, Me.txtEffectiveTill, Me.txtExtensionNo
    modControlsSettings.setStateToNormal Me.txtEffectiveFrom, Me.txtEffectiveTill, Me.txtExtensionNo
End Sub
...
Private Sub txtExtensionNo_GotFocus()
    uiManager.setStatusText ""
End Sub


Private Sub txtExtensionNo_Validate(Cancel As Boolean)
    validateExtensionNo
End Sub
