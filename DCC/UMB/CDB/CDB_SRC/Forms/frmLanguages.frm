...
Begin VB.Form frmLanguages 
   ...
   Begin VB.Frame fraLanguages 
      ...
      Top             =   0
      Width           =   13695
      Begin FPSpread.vaSpread sprLanguages 
         Height          =   5925
         Left            =   360
         TabIndex        =   1
         Top             =   360
         ...
         SpreadDesigner  =   "frmLanguages.frx":0000
      End
   End
End
...
Dim WithEvents sprLanguagesController As clsSpreadController
Attribute sprLanguagesController.VB_VarHelpID = -1

'This enum holds all the column numbers of the spread controller 'sprLanguagesController' that
...
On Error GoTo errortag
    ...
    If Not sprLanguagesController.prepareSpreadForSave Then
        Exit Sub
    End If
    ...
    saveData = False
    strDeleteTemplate = "CDB..delLanguages " & "@@, @@"
    strUpdateTemplate = "CDB..updLanguages " & "@@, @@, @@, @@, @@, @@"
    strInsertTemplate = "CDB..insLanguages " & "@@, @@, @@, @@"
    
    dbManager.beginTrans
    Dim i As Long
    Dim sqlStrings() As String
    If sprLanguagesController.getNumberOfRowsForState(DELETED_STATE) <> 0 Then
         
        sqlStrings = sprLanguagesController.getDeleteStrings(strDeleteTemplate, _
                                                                      "Yes", _
                                                                      "No ", _
                                                                      languagesColumns.ID, _
                                                                      languagesColumns.txn_ts)
        ...
        Next i
    End If
    If sprLanguagesController.getNumberOfRowsForState(UPDATED_STATE) <> 0 Then
         sqlStrings = sprLanguagesController.getUpdateStrings(strUpdateTemplate, _
                                                                      "Yes", _
                                                                      "No ", _
                                                                      languagesColumns.ID, _
                                                                      languagesColumns.code, _
                                                                      languagesColumns.Name, _
                                                                      languagesColumns.effectivefrom, _
                                                                      languagesColumns.effectiveTill, _
                                                                      languagesColumns.txn_ts)
        ...
        Next i
    End If
    If sprLanguagesController.getNumberOfRowsForState(NEW_STATE) <> 0 Then
         sqlStrings = sprLanguagesController.getInsertStrings(strInsertTemplate, _
                                                                      "Yes", _
                                                                      "No ", _
                                                                      languagesColumns.code, _
                                                                      languagesColumns.Name, _
                                                                      languagesColumns.effectivefrom, _
                                                                      languagesColumns.effectiveTill)
...
'retrieve the existing records of skills to the spread controller "sprLanguagesController"
Private Sub retrieveLanguages()
On Error GoTo errortag
    Dim strSQL As String
    strSQL = "SELECT language_cd, language_nm, Cast(txn_ts As BigInt), Convert(varchar, effective_from, 106)effective_from, " & _
             "Convert(varchar,effective_till,106)effective_till, language_id " & _
             "From CDB..languages"
    sprLanguagesController.retrieveRecords strSQL
Exit Sub
...
Public Function clsWindowInterface_getScreenID() As String
    clsWindowInterface_getScreenID = "MIS_05"
End Function
...
Private Sub setSpreadController()
    ...
    'initializing and adding columns to sprLanguagesController of skills
    Set sprLanguagesController = New clsSpreadController
    sprLanguagesController.registerSpreadInstance uiManager, sprLanguages, TABULAR_SPREAD, Me
    sprLanguagesController.setHeaderHeight 500
    
    sprLanguagesController.addColumn "Code", 10, edit_CELL_TYPE, True, True, , , , 20
    sprLanguagesController.addColumn "Language", 50, edit_CELL_TYPE, True, True, , , , 20
    sprLanguagesController.addColumn "Txn Ts", 0, STATIC_TEXT_CELL_TYPE, NON_EDITABLE_COLUMN, NON_MANDATORY_COLUMN
    sprLanguagesController.addColumn "Effective From", 15, DATE_CELL_TYPE, True, True, , , , , CENTER_ALIGNMENT
    sprLanguagesController.addColumn "Effective Till", 15, DATE_CELL_TYPE, True, True, , , , , CENTER_ALIGNMENT
    sprLanguagesController.addColumn "ID", 9, STATIC_TEXT_CELL_TYPE, False, False, , , , , RIGHT_ALIGNMENT
...
Private Sub setEnableDisableControls(p_intMode As Integer)
    Select Case p_intMode
        Case uiManager.TABULAR_MODE
            sprLanguagesController.setSpreadModeForTabularSpread p_intMode
        Case uiManager.NEW_MODE
            sprLanguagesController.setSpreadModeForTabularSpread p_intMode
    End Select
End Sub

Private Sub sprLanguagesController_setDefaultValues(ByVal p_lonRow As Long)
    sprLanguagesController.setTextOfCell p_lonRow, languagesColumns.effectivefrom, Format(Date, "dd mmm yyyy")
    sprLanguagesController.setTextOfCell p_lonRow, languagesColumns.effectiveTill, "31 Dec 9999"
End Sub
...
Private Sub sprLanguagesController_ValidateCell(ByVal p_lonRow As Long, _
...
On Error GoTo errortag
    ...
    varEffectiveFrom = sprLanguagesController.getTextOfCell(p_lonRow, languagesColumns.effectivefrom)
    varEffectivetill = sprLanguagesController.getTextOfCell(p_lonRow, languagesColumns.effectiveTill)
    
    Select Case p_lonColumn
        Case languagesColumns.effectiveTill
        ...
        Case languagesColumns.effectivefrom
        ...
        Case languagesColumns.Name
            If Trim(sprLanguagesController.getTextOfCell(p_lonRow, p_lonColumn)) = "" Then
                sprLanguagesController.setTextOfCell p_lonRow, p_lonColumn, ""
            End If
            
        Case languagesColumns.code
            If Trim(sprLanguagesController.getTextOfCell(p_lonRow, p_lonColumn)) = "" Then
                sprLanguagesController.setTextOfCell p_lonRow, p_lonColumn, ""
            ElseIf Len(Trim(sprLanguagesController.getTextOfCell(p_lonRow, p_lonColumn))) > 2 Then
                logger.logMessage modStandardMessages.MaxResources, "2 characters|Code", WARNING_MESSAGE, Me.clsWindowInterface_getScreenID, Me.clsWindowInterface_getMode
                p_bolIsCellValid = False
            End If
...
errortag:
    errorManager.setErrorAttributes modStandardMessages.UnknownError, "sprLanguagesController_ValidateCell", "", ERROR_TYPE, Me.clsWindowInterface_getScreenID, Me.clsWindowInterface_getMode
    Err.Raise Err.Number
End Sub


Private Sub sprLanguagesController_ValidateRow(ByVal p_lonRow As Long, p_bolIsRowValid As Boolean)
    Dim varEffectiveFrom As Variant
    Dim varEffectivetill As Variant
    ...
    If sprLanguagesController.isCellValueRepeatedInColumn(utils.createLongArray(languagesColumns.Name), p_lonRow, True) Then
